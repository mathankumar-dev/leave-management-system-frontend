    ...
    useEffect(() => {
        ...
        const fetchBalance = async () => {
            try {
                ...
                setBalance(res.data as LeaveBalance);
            } catch {
                            ...
                            <div className="bg-amber-50 rounded-xl p-4 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-400">
                                        Earned: {balance.compOffEarned} · Used: {balance.compOffUsed}
                                    </p>
                                </div>
...
// ─── Helper ───────────────────────────────────────────────────────
// employeeId is always string — no number coercion
...
const CreatePayslipModal: React.FC<{ employee: Employee; onClose: () => void }> = ({ employee, onClose }) => {
    ...
    const [form, setForm] = useState<PayslipCreateRequest>({
        employeeId:     employee.id,   // string — correct
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Try exact payslip (edit mode)
                try {
                    const res = await api.get(
                        `/payslip/employee/${employee.id}/${formYear}/${formMonth}`
                    );
                    setExistingPayslip(res.data as Payslip);
                    setForm(fillFormFromData(res.data as Payslip, employee.id, formMonth, formYear));
                    return;
                } catch {
                    setExistingPayslip(null);
                }

                // Try prefill (create mode with previous data)
                try {
                    const res = await api.get(
                        `/payslip/prefill?employeeId=${employee.id}&year=${formYear}&month=${formMonth}`
                    );
                    setForm(fillFormFromData(res.data as Partial<PayslipCreateRequest>, employee.id, formMonth, formYear));
    ...
    // Separate updaters to keep string vs number types clean
...
export const CFOEmployeesPage: React.FC = () => {
    ...
    // employeeMap keyed by string id, value is Employee (not EmployeeEntity)
    const [employeeMap, setEmployeeMap]   = useState<Record<string, Employee>>({});

    const load = async (page = 0) => {
        try {
            setLoading(true);
            // getAllEmployeesHR returns EmployeePageResponse with content: Employee[]
            ...
            // Build id→Employee map (id is string)
            setEmployeeMap(prev => {
                const updated = { ...prev };
    ...
    // managerId is string | null — match against employeeMap string keys
    ...
    const toggleTaxRegime = (id: string) => {
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                {loading ? (
                    <div className="flex justify-center py-16">
                        <CustomLoader label="Loading employees..." />
                    </div>
                ) : filtered.length === 0 ? (
