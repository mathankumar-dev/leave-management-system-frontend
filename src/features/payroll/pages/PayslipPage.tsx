...
const PayslipFormModal: React.FC<{
  ...
  // employeeId is string throughout — no parseInt needed
  ...
  // Separate updaters for string fields vs number fields
  ...
  const gross =
    form.basicSalary + form.hra + form.conveyance + form.medical +
    form.otherAllowance + form.bonus + form.incentive + form.stipend;
  const deductions =
    form.pf + form.esi + form.professionalTax + form.tds + form.variablePay + form.lop;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-3 gap-4">
            {/* Employee select — value is string id */}
            <div className="space-y-1.5 col-span-1">
              ...
              <select
                ...
                onChange={e => updateStr('employeeId', e.target.value)}
                disabled={mode === 'edit'}
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={form.month}
                onChange={e => updateNum('month', parseInt(e.target.value, 10))}
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={form.year}
                onChange={e => updateNum('year', parseInt(e.target.value, 10))}
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              >
        ...
        <div className="flex gap-3 p-6 pt-0 sticky bottom-0 bg-white border-t border-slate-100">
          ...
          <button
            onClick={() => {
              if (!form.employeeId) {
                notify.error('Missing', 'Select an employee');
                return;
              }
              onSave(form);
            }}
...
export const PayslipPage: React.FC<PayslipPageProps> = ({
  ...
  // Fetch employees and build id→Employee map
  useEffect(() => {
    const load = async () => {
      try {
        // getAllEmployees returns PaginatedResponse<EmployeeEntity>
        // but we need the Employee shape used throughout this page.
        // employeeService.getAllEmployeesHR returns Employee[] directly.
  ...
  // Auto-select employee when navigated from CFOEmployeesPage
  useEffect(() => {
  ...
  // Fetch payroll whenever month/year changes
  useEffect(() => {
    fetchPayrollData(year, month);
  }, [year, month, fetchPayrollData]);

  const getEmpName = (row: Payslip): string =>
  ...
  const totalDeductions = payrollData.reduce(
    (sum, p) => sum + (p.pf ?? 0) + (p.esi ?? 0) + (p.professionalTax ?? 0) + (p.tds ?? 0),
    0
  );
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  {['Employee','Basic','HRA','Bonus','Incentive','Stipend','Gross','PF','TDS','Prof.Tax','LOP','Net','Actions'].map(h => (
                    <th
                      key={h}
                      className={`py-3 px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest ${
                        h === 'Employee' ? 'text-left' : 'text-right'
                ...
                {filtered.map(row => (
                  <tr key={row.employeeId} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-3 text-right text-slate-600">
                      ₹{(row.basicSalary ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-slate-600">
                      ₹{(row.hra ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-emerald-600 font-semibold">
                      ₹{(row.bonus ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-emerald-600 font-semibold">
                      ₹{(row.incentive ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-emerald-600 font-semibold">
                      ₹{(row.stipend ?? 0).toLocaleString('en-IN')}
                    </td>
                    ...
                    <td className="py-3 px-3 text-right text-rose-500">
                      ₹{(row.pf ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-rose-500">
                      ₹{(row.tds ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-rose-500">
                      ₹{(row.professionalTax ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right text-rose-500">
                      ₹{(row.lop ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3 text-right font-black text-slate-800">
                      ₹{(row.netSalary ?? 0).toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-3">
