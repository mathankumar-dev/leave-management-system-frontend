...
const PayrollView = () => {
  ...
  console.log("PAYSLIP STATE:", payslip); 
 ...
 useEffect(()=>{
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white border shadow p-8">
      ...
      {!payslip && <div className="text-center text-gray-500">No Payslip</div>}

      {payslip && (
        <>
          <div className="grid grid-cols-2 gap-4 text-sm mb-6">
            <Info label="Employee ID" value={payslip.employeeId}/>
            <Info label="Month" value={`${payslip.month}/${payslip.year}`}/>
          </div>
          ...
          <table className="w-full border text-sm mb-6">
            ...
            <tbody>
              <Row left="Basic" leftVal={payslip.basicSalary} right="PF" rightVal={payslip.pf}/>
              <Row left="HRA" leftVal={payslip.hra} right="ESI" rightVal={payslip.esi}/>
              <Row left="Conveyance" leftVal={payslip.conveyance} right="Professional Tax" rightVal={payslip.professionalTax}/>
              <Row left="Medical" leftVal={payslip.medical} right="LOP" rightVal={payslip.lop}/>
              <Row left="Other Allowance" leftVal={payslip.otherAllowance} right="" rightVal=""/>
            </tbody>
          </table>
          ...
          <button
            onClick={()=>download(payslip.year,payslip.month)}
